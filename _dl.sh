...
[
  ...
  },
  {
    ...
    "sig": ".asc",
    "keys_comment": "Via: https://c-ares.org/",
  ...
  },
  {
    ...
    "keys": "B146647E45A7B33947AB226B2A2C87D161692D40 7953AC1FBC3DC8B3B292393ED5E9E43F7DF9EE8C 8657ABB260F056B1E5190839D9C4D26D0E604491 B7C1C14360F353A36862E4D5231C84CDDCC69C45 A21FAB74B0088AA361152586B8EF1A6BA9DA2D5C"
  },
  {
    "name": "libressl",
...
my_curl() {
  ...
  opts=(--disable --user-agent '' --fail --silent --show-error --globoff \
    --remote-time --xattr \
    --connect-timeout 15 --max-time 80 --retry 6 --max-redirs 10)
  # >&2 echo "my_curl|${opts[*]} $*|"
  ...
  if [[ -z "${CW_GET:-}"   || " ${CW_GET} "    = *" ${name} "* ]] && \
    ...
    if [[ "${url}" =~ ^https://raw.githubusercontent.com/([a-zA-Z0-9_.-]+/[a-zA-Z0-9_.-]+)/[^/]+(/.+)$ ]]; then
      ...
      if [ "${gittar}" = '1' ]; then
        ...
        dcommit="${ver} 00:00:00"  # YYYY-MM-DD hh:mm:ss
        echo "! Using file timestamp: |${dcommit}|"
      fi
    fi
