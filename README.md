...
> making this a good time to update to a recent, safe version.**
  ...
  - curl/libcurl features enabled by default (`{upcoming}`):
    ...
    <br>`AsynchDNS HSTS HTTP2 HTTPS-proxy IDN IPv6 Kerberos Largefile MultiSSL NTLM SPNEGO SSL SSPI TLS-SRP Unicode UnixSockets alt-svc brotli libz zstd {gsasl}`
