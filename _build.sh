...
#        canonsrc   prefer the canonical URL for source tarballs (vs. the mirror)
...
build_single_target() {
    ...
    if [ "${_OS}" = 'linux' ]; then
      ...
      if [ "${_HOST}" = 'linux' ] && [ "${_OS}" = 'linux' ]; then
        ...
        if [ "${unamem}" = "${_machine}" ]; then
          _RUN_BIN=''
        elif [ "${_DISTRO}" = 'debian' ] && [ "${_CRT}" != 'gnu' ] && command -v "qemu-${_machine}-static" >/dev/null 2>&1; then
  ...
  if [ "${_OS}" = 'win' ]; then
    ...
    # macOS 10.14 Mojave 2018-09-24. Seems to work for arm64 builds,
