...
(
  ...
  # public libcurl functions being marked as 'exported'. It is useful to
    ...
    # To avoid this error (apparently this option assumes MSVC compiler unconditionally):
    #   clang-12: error: no such file or directory: '/MT'
  # options="${options} -DCURL_STATIC_CRT=1"
    options="${options} -DCURL_WINDOWS_SSPI=1"
    if [ -d ../zlibng ]; then
      options="${options} -DZLIB_INCLUDE_DIR:PATH=$(pwd)/../zlibng/pkg/usr/local/include"
      options="${options} -DZLIB_LIBRARY:FILEPATH=$(pwd)/../zlibng/pkg/usr/local/lib/libz.a"
    else
